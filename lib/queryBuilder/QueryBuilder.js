...
class QueryBuilder extends QueryBuilderBase {
  ...
  tableNameFor(modelClass, tableName) {
    ...
    if (isString(tableName)) {
      ...
      ctx.tableMap.set(modelClass.getTableName(), tableName);
      return this;
    } else {
      return (tableMap && tableMap.get(modelClass.getTableName())) || modelClass.getTableName();
    }
  }
  ...
  aliasFor(modelClass, alias) {
    ...
    if (isString(alias)) {
      ...
      ctx.aliasMap.set(modelClass.getTableName(), alias);
      return this;
    } else {
      return (aliasMap && aliasMap.get(modelClass.getTableName())) || null;
    }
  }
  ...
  eager(exp, filters) {
    this._eagerExpression = parseRelationExpression(this._modelClass, exp);
  ...
  mergeEager(exp, filters) {
    ...
    this._eagerExpression = this._eagerExpression.merge(
      parseRelationExpression(this._modelClass, exp)
    );
  ...
  allowEager(exp) {
    this._allowedEagerExpression = parseRelationExpression(this._modelClass, exp);
  ...
  isSelectAll() {
    ...
    const tableName = this.tableNameFor(this.modelClass());
  ...
  columnInfo({ table = null } = {}) {
    table = table || this.tableNameFor(this.modelClass());
