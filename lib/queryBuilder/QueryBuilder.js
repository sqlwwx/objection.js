...
const { defineNonEnumerableProperty } = require('../model/modelUtils');
...
class QueryBuilder extends QueryBuilderBase {
  ...
  tableNameFor(modelClassOrTableName, newTableName) {
    let tableName = modelClassOrTableName;

    if (!isString(modelClassOrTableName)) {
      tableName = modelClassOrTableName.getTableName();
    }

    return super.tableNameFor(tableName, newTableName);
  }
  ...
  aliasFor(modelClassOrTableName, alias) {
    ...
    return super.aliasFor(tableName, alias);
  }
  ...
  applyFilter() {
    const namedFilters = this.modelClass().namedFilters;
  ...
  joinEager(exp, filters) {
    return this.eagerAlgorithm(this.modelClass().JoinEagerAlgorithm).eager(exp, filters);
  }

  naiveEager(exp, filters) {
    return this.eagerAlgorithm(this.modelClass().NaiveEagerAlgorithm).eager(exp, filters);
  }
  ...
  mergeEager(exp, filters) {
    ...
    this._eagerFilters = Object.assign({}, this._eagerFilters, filters);
  ...
  mergeJoinEager(exp, filters) {
    return this.eagerAlgorithm(this.modelClass().JoinEagerAlgorithm).mergeEager(exp, filters);
  }

  mergeNaiveEager(exp, filters) {
    return this.eagerAlgorithm(this.modelClass().NaiveEagerAlgorithm).mergeEager(exp, filters);
  }
  ...
  modifyEager(path, filter) {
    this._eagerFiltersAtPath.push({ path, filter });
    return this;
  }
  ...
  clone() {
    ...
    builder._eagerFiltersAtPath = this._eagerFiltersAtPath.slice();
...
function addEagerFetchOperation(builder) {
  if (!builder.has(EagerOperation) && builder._eagerExpression) {
    ...
    const filters = Object.assign({}, builder._eagerFilters);

    builder._eagerFiltersAtPath.forEach((filter, i) => {
      const filterName = `_f${i}_`;

      expression.expressionsAtPath(filter.path).forEach(expr => {
        expr.rawNode.$modify.push(filterName);
      });

      filters[filterName] = filter.filter;
    });

    builder.addOperation(operation, [expression, filters]);
  }
}
  ...
  if (Array.isArray(result)) {
    if (result.length && shouldBeConvertedToModel(result[0], modelClass)) {
      for (let i = 0, l = result.length; i < l; ++i) {
        ...
        defineNonEnumerableProperty(result[i], '$$queryContext', queryContext);
      }
    }
  } else if (shouldBeConvertedToModel(result, modelClass)) {
    result = modelClass.fromDatabaseJson(result);
    defineNonEnumerableProperty(result, '$$queryContext', queryContext);
  }
