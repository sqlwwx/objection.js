...
class Model {
  ...
  $toJson(opt) {
    return toJson(this, !!(opt && opt.shallow));
  }
  ...
  static fetchDbMetadata({ parentBuilder = null, knex = null } = {}) {
    // Memoize metadata but only for this class. The hasOwnProperty check
    // will fail for subclasses and the value gets recreated.
    if (!this.hasOwnProperty('$$dbMetadata')) {
      defineNonEnumerableProperty(this, '$$dbMetadata', new Map());
    }
    ...
    const metadata = this.$$dbMetadata.get(table);

    if (metadata) {
      return Promise.resolve(metadata);
    } else {
      const promise = DbMetadata.fetch({ modelClass: this, parentBuilder, knex }).then(metadata => {
        this.$$dbMetadata.set(table, metadata);
        return metadata;
      });

      this.$$dbMetadata.set(table, promise);
      return promise;
    }
  }

  static getDbMetadata(builder = null) {
    // The table isn't necessarily same as `this.getTableName()` for example if
    // a view is queried instead.
    const table = (builder && builder.tableNameFor(this)) || this.getTableName();
